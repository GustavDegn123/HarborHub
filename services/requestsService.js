...
/**
 * Live-lyt til ejerens ÅBNE (status === "open") requests.
 * Brug denne til “Bud på mine opgaver”, så accepterede/betalte forsvinder.
 */
export function listenOwnerOpenRequestsWithBids(
  ownerId,
  callback,
  { onlyWithBids = true } = {}
) {
...
/** Engangskald: ejerens ÅBNE requests */
export async function getOwnerOpenRequestsWithBids(
  ownerId,
  { onlyWithBids = true } = {}
) {
  if (!ownerId) return [];
